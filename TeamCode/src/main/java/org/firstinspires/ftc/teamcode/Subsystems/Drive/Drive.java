...
public class Drive extends Subsystem {
    ...
    public void motorController(Targeter targeter, HolonomicController positionController) {
        ...
        Pose previousPosition = currentPose;
        ...
        while (!targeter.reachedTarget(currentPose) && (!timeoutManager.isExceeded())) {
            ...
            Pose targetPose = targeter.getTarget(currentPose);
    ...
    public void followProfile(MotionProfile profile, HolonomicController positionController) {
        ...
        while (!profile.isFinished(timer.seconds()) && !timeoutManager.isExceeded()) {
            poseEstimator.update();
            currentPose = poseEstimator.getPose();
            org.firstinspires.ftc.teamcode.Subsystems.Drive.MotionProfile.MotionProfileOutput target = profile.calculate(timer.seconds());
            HolonomicControllerOutput positionMotorPowers = positionController.calculate(currentPose, new Pose(target.x.position,
                    target.y.position,
