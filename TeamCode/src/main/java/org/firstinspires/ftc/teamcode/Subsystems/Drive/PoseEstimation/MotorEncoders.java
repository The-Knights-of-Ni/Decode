...
public class MotorEncoders implements PoseEstimationMethod {
    ...
    @Override
    public void update() {
        double forwardSpeed = (motors.frontLeft.getVelocity() + motors.frontRight.getVelocity() + motors.rearLeft.getVelocity() + motors.rearRight.getVelocity()) / 4;
        double strafeSpeed = (-motors.frontLeft.getVelocity() + motors.frontRight.getVelocity() + motors.rearLeft.getVelocity() - motors.rearRight.getVelocity()) / 4;
        double turnSpeed = (motors.rearRight.getVelocity() + motors.frontRight.getVelocity() - motors.frontLeft.getVelocity() - motors.rearLeft.getVelocity()) / (4 * 2 * rb);
        double now = timer.milliseconds();
        double dt = now - lastTime;
        double forwardDistance = forwardSpeed * dt;
        double strafeDistance = strafeSpeed * dt;
        double turnDistance = turnSpeed * dt;
