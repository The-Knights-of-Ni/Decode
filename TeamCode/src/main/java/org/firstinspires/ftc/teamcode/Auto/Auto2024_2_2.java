...
@Autonomous(name = "Auto2024_2_2")
public class Auto2024_2_2 extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        move(new Vector2D(5*mmPerInch, 10*mmPerInch), 42.5); // turn to have wall facing element to push
        move(new Vector2D(28*mmPerInch, 0), -5.5); // scoot towards OZ
        move(new Vector2D(-15*mmPerInch, 0), -3); // scoot out of OZ
        clawOpen(c); // open claw
        move(new Vector2D(0, 0), -43); // turn left
        move(new Vector2D(0, -18*mmPerInch),  -17.5); // scoot back to OZ
        clawClose(c); // close claw
        Thread.sleep(1000);
        move(new Vector2D(0, 1*mmPerInch), 5); // scoot slightly out of OZ
        setPosition(al, ar, Pivot.HIGHBAR); // move to carry position

        move(new Vector2D(-30*mmPerInch, 4), 7.5); // scoot left
        ...
        // TODO: Add code to pick up specimen and hang on bar
